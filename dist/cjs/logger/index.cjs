...
var Logger = /** @class */ (function () {
  function Logger(options) {
    ...
    this.timestamps =
      (_a = options === null || options === void 0 ? void 0 : options.timestamps) !==
        null && _a !== void 0
        ? _a
        : true;
    this.messageFunctions = __assign(
      {
  ...
  Logger.prototype.createMessage = function (options) {
    var timestamp = options.timestamp,
      ...
      color = options.color;
    var coloredLevel = (0, utils_1.addColorToText)(
      color,
      '['.concat(level.toUpperCase(), ']'),
    );
    return ''
      .concat(timestamp.length > 0 ? '['.concat(timestamp, ']') : '')
      .concat(coloredLevel, ' ')
      .concat(message);
  };
  Logger.prototype.displayLog = function (level, message) {
    if (level === 'notification') {
      var noti = '\n['
        .concat((0, utils_1.getTimestamp)(), '] [')
        .concat((0, utils_1.addColorToText)('#73FBD3', 'NOTIFICATION'), '] ')
        .concat(message, '\n');
      process.stdout.write(noti);
      return;
    }
    var preparedMessage = this.messageFunctions[level]({
      timestamp: this.timestamps ? ''.concat((0, utils_1.getTimestamp)()) : '',
      level: level,
      message: message,
  ...
  Logger.prototype.export = function (filePath, options) {
    ...
    var levels = options.levels,
      _a = options.format,
      format = _a === void 0 ? 'txt' : _a,
      _b = options.jsonSpaces,
      jsonSpaces = _b === void 0 ? 2 : _b,
      _c = options.csvSeparator,
      csvSeparator = _c === void 0 ? ',' : _c,
      _d = options.includeTimestamp,
      includeTimestamp = _d === void 0 ? true : _d,
      _e = options.includeLevel,
      includeLevel = _e === void 0 ? true : _e;
    ...
    if (levels && (levels === null || levels === void 0 ? void 0 : levels.length) > 0) {
      filteredLogs = this.logs.filter(function (log) {
        var _a;
        var logLevel =
          (_a = log.match(/\[([A-Z]+)\]/)) === null || _a === void 0
            ? void 0
            : _a[1].toLowerCase();
    ...
    switch (format) {
      case 'json': {
        var jsonLogs = filteredLogs.map(function (log) {
          var _a = log.split('] '),
            timestamp = _a[0],
            level = _a[1],
            messageParts = _a.slice(2);
          var message = messageParts.join('] ').trim();
          return __assign(
            __assign(
              __assign({}, includeTimestamp && { timestamp: timestamp.slice(1) }),
      ...
      case 'csv': {
        var csvHeaders = __spreadArray(
          __spreadArray(
            __spreadArray([], includeTimestamp ? ['Timestamp'] : [], true),
            includeLevel ? ['Level'] : [],
            true,
          ),
          ['Message'],
        ...
        var csvRows = filteredLogs.map(function (log) {
          ...
          return __spreadArray(
            __spreadArray(
              __spreadArray([], includeTimestamp ? [timestamp.slice(1)] : [], true),
